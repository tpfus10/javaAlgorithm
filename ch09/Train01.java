...
class Tree5 {
	TreeNode5 root; //TreeNode의 참조변수 root

	Tree5() {
		root = null;
	}

	TreeNode5 inorderSucc(TreeNode5 current) {//inorder successor를 찾는 메소드
	...
	void inorder() { //inorder 오버로딩1(매개변수 없음)
		inorder(root);
	}
	...
	//inorder 오버로딩2(매개변수 있음)
	void inorder(TreeNode5 CurrentNode) {//재귀함수(root에서 시작해서 왼쪽-중간-오른쪽 순으로 출력을 반복함)
		if (CurrentNode != null) {
			inorder(CurrentNode.LeftChild);//inorder1이 아니라 2로 감
	...
	//stack을 사용한 inorder 출력
	{
		...
		while (true) {
			...
			if (!s.isEmpty()) {
				try {
					CurrentNode = s.pop();
				} catch (ch09.ObjectStack5.EmptyGenericStackException e) {
	...
	boolean insert(int x) {// binary search tree를 만드는 입력 : left subtree < 노드 x < right subtree
		...
		TreeNode5 p = root; //previous(q는 p를 따라다님)
		TreeNode5 q = null; //present
...
//-------------------------------------------------------------------------------------------main
	...
	public static void main(String[] args) {
		...
		Scanner stdIn = new Scanner(System.in);
		Tree5 t = new Tree5(); //Tree5   객체의 참조변수 t
		Menu menu; // 메뉴
		int count = 20;
		int num;
		boolean result;
		do {
			...
			case Delete: //임의 정수 삭제(어려움, 교재 376~378)
